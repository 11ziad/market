...
const markMessagesAsRead = async () => {
  ...
  console.log('عدد الرسائل غير المقروءة بعد التحديث:', count)
  ...
  const fetchContacts = async () => {
    ...
    users.forEach(user => {
      const lastSent = sentMessages?.filter(m => m.receiver_id === user.id).sort((a, b) => new Date(b.created_at) - new Date(a.created_at))[0]
      const lastReceived = receivedMessages?.filter(m => m.sender_id === user.id).sort((a, b) => new Date(b.created_at) - new Date(a.created_at))[0]
    ...
    const sortedContacts = Object.values(userMap).sort((a, b) => new Date(b.lastTime) - new Date(a.lastTime))
...
useEffect(() => {
  ...
  const loadMessages = async () => {
    // 1. علّم الرسائل إنها اتقرأت
    await markMessagesAsRead(otherUserId)

    // 2. انتظر شوية علشان Supabase يخلص التحديث
    await new Promise(resolve => setTimeout(resolve, 300))

    // 3. احسب عدد الرسائل غير المقروءة من Supabase
    const { count } = await supabase
      .from('messages')
      .select('*', { count: 'exact', head: true })
      .eq('receiver_id', currentUser.id)
    ...
    // 4. خزّن العدد في localStorage علشان Navbar يقرأه
localStorage.setItem('forceUnreadCount', count || 0)

    // 5. كمّل تحميل الرسائل والمستخدمين
    await fetchMessages()
    await fetchContacts()
  }
      ...
      <>
        <Box flex={1} p={2} overflow="auto">
          {messages.map(msg => {
            ...
            const imageUrl = hasImage ? msg.message.split('صورة المنتج:\n')[1]?.trim() : null
            const textOnly = hasImage ? msg.message.split('صورة المنتج:')[0].trim() : msg.message
            ...
            const bubbleColor = isSender ? '#fff' : theme.palette.text.primary

            return (
              <Box key={msg.id} sx={{ textAlign: isSender ? 'right' : 'left', mb: 2 }}>
                <Box
                  sx={{
                ...
                >
                  ...
                  {isSender && (
                    <span style={{ marginLeft: 8 }}>
                      {msg.is_read ? <DoneAllIcon fontSize="small" /> : <DoneIcon fontSize="small" />}
                    </span>
                  )}
                </Box>
              </Box>
