...
import {
  Box, Typography, Avatar, CircularProgress, Dialog,
  DialogTitle, DialogContent, IconButton, Button, TextField, Chip,
  Card, CardContent, Divider, Slider,
  useMediaQuery
} from '@mui/material'
...
useEffect(() => {
  ...
  const handleAddComment = async () => {
    ...
    if (authError || !user) { setSending(false); return }
  ...
  const handleAddToCart = async (product) => {
    ...
    await supabase.from('cart_items').insert({ user_id: user.id, product_id: product.id })
...
const handleHideProduct = (productId) => {
  ...
  const closeProductModal = () => { setSelectedProduct(null); setComments([]); setCommentText('') }
...
>
  {/* ✅ زر إغلاق الفلاتر في الشاشات الصغيرة */}
  {isSmallScreen && (
    <IconButton
  ...
  <TextField
    ...
    sx={{
      ...
      '& .MuiInputBase-root': {
        borderRadius: 2,
        backgroundColor: theme.palette.mode === 'dark' ? '#3a3a3a' : '#f7f7f7',
        color: theme.palette.text.primary,
        '&:hover': {
...
>
  ...
  <Box sx={{ flex: 1, [isArabic ? 'mr' : 'ml']: { md: '250px' } }}>
    ...
    <Box flex={1}>
      ...
      {filteredProducts.length === 0 && <Typography>📭 لا توجد منتجات</Typography>}
...
<Box
  ...
  sx={{
    ...
    overflow: 'hidden',
    bgcolor: theme.palette.mode === 'dark' ? '#2a2a2a' : '#edededff',
    color: theme.palette.text.primary,
...
>
  ...
  <Box
    ...
    sx={{
      ...
      filter: theme.palette.mode === 'dark' ? 'brightness(0.9)' : 'brightness(0.97)',
      '&:hover': {
    ...
    <Box display="flex" gap={1} flexWrap="wrap">
      {p.category && (
        <Chip
          ...
          size="small"
          sx={{
            bgcolor: theme.palette.mode === 'dark' ? '#3a3a3a' : '#e3f2fd',
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Typography variant="h6" fontWeight="bold" mt={2} sx={{ color: theme.palette.text.primary }}>
            <MessageIcon sx={{ fontSize: 16 }} /> {t('Comments')}
          </Typography>

          {comments.length === 0 ? (
            <Typography color="text.secondary">{t('Therearenocommentsyet')}</Typography>
          ) : (
      ...
      [theme.breakpoints.down('sm')]: {
        ...
        {/* ✅ جهة صاحب المنتج (للشاشات الكبيرة فقط) */}
        <Box
          width={{ xs: '100%', md: 300 }}
        ...
        >
          <Avatar
            src={selectedProduct.profiles?.avatar_url || '/default-avatar.png'}
            sx={{ width: 72, height: 72, border: `2px solid ${theme.palette.background.paper}` }}
          />
          ...
          <Button
            ...
            sx={{ borderRadius: 3 }}
            onClick={() => navigate(`/profiledetails/${selectedProduct.profiles?.id}`)}
          >
            {t('Profile')}
          </Button>
          <Button
...
>
  ...
  >
    {t('send')}
      ...
      {/* ✅ Dialog صغير لصاحب المنتج في الشاشات الصغيرة */}
      <Dialog
