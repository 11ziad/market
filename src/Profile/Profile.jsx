...
import {
  Box, Typography, Avatar, Button, Tabs, Tab,
  ...
  if (!error) {
    ...
    const updated = products.map(p =>
      p.id === selectedProduct.id
        ? { ...p, ...editValues, price: parseFloat(editValues.price), image_url: imageUrl }
        : p
    )
    setProducts(updated)
  } else {
    ...
    <Box p={3}>
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <Box textAlign="center" mb={3}>
          <Avatar src={profile.avatar_url || '/default-avatar.png'} sx={{ width: 120, height: 120, margin: 'auto' }} />
        ...
        <Tabs
          ...
          sx={{
            mb: 3,
            "& .MuiTab-root": { fontWeight: "bold", fontSize: "16px", transition: "0.3s" },
            "& .Mui-selected": { color: "#1976d2" },
            "& .MuiTabs-indicator": { height: "4px", borderRadius: "2px", backgroundColor: "#1976d2" }
          }}
        >
      ...
      {tabIndex === 0 && (
        <Box>
          <Typography variant="h6" mb={2}>{t('PortWorks')} :</Typography>
    ...
    <Tooltip title="حذف">
      <IconButton
        ...
        sx={{
          bgcolor: theme.palette.mode === 'dark' ? '#444' : '#f5f5f5',
          '&:hover': {
            bgcolor: theme.palette.mode === 'dark' ? '#d32f2f' : '#ffebee'
          }
        }}
      >
    ...
    <Tooltip title="عرض التعليقات">
      <IconButton
        ...
        sx={{
          ...
          '&:hover': {
            bgcolor: theme.palette.mode === 'dark' ? '#555' : '#e0f7fa'
          },
          width: 40,
          height: 40,
          borderRadius: 2,
      ...
      >
        <ChatBubbleOutline sx={{ fontSize: 20, color: theme.palette.primary.main }} />
      </IconButton>
    </Tooltip>
  </Box>
</Box>
  ...
  <CardContent sx={{ display: 'flex', gap: 2, padding: 0 }}>
    ...
    <Box flex={1}>
      <Box display="flex" justifyContent="space-between" alignItems="center">
        ...
        {/* ✅ زر الحذف لو المستخدم هو صاحب التعليق */}
    ...
    sx={{
      ...
      boxShadow: theme.palette.mode === 'dark'
        ...
        : '0 6px 20px rgba(0,0,0,0.08)',
                      backgroundColor: theme.palette.mode === 'dark' ? ' #121212' : '#edededff',
  ...
  >
    ...
    <Box mt={3}>
      ...
      <Box
        sx={{
          ...
          p: 3,
          textAlign: "center",
          bgcolor: theme.palette.mode === 'dark' ? '#1e1e1e' : '#f9fcff'
        }}
      >
    ...
    <Box mt={4} textAlign="center">
      ...
      <Dialog open={!!selectedProduct} onClose={() => setSelectedProduct(null)} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          <TextField label={t('Productname')} value={editValues.name} onChange={(e) => setEditValues({ ...editValues, name: e.target.value })} fullWidth margin="normal" />
          <TextField label={t('Description')} value={editValues.description} onChange={(e) => setEditValues({ ...editValues, description: e.target.value })} fullWidth margin="normal" />
          <TextField label={t('price')} type="number" value={editValues.price} onChange={(e) => setEditValues({ ...editValues, price: e.target.value })} fullWidth margin="normal" />
          ...
          {newImagePreview && (<Avatar src={newImagePreview} sx={{ width: '100%', height: 200, mt: 2 }} variant="square" />)}
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setSelectedProduct(null)} color="secondary">{t('cancel')}</Button>
          <Button onClick={handleSaveProductEdit} variant="contained" color="primary">{t('Savemodifications')}</Button>
        </DialogActions>
      </Dialog>
    </Box>
  )
